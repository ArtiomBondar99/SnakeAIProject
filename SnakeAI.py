...
def saveQtable(fileName="qTable.pkl"):
    with open(fileName,"wb") as f:
        pickle.dump(q_table,f)


def loadQtable(filename="q_table.pkl"):
    global q_table
    try:
...
def run_episode(getNextDirectionState, perform_action):
    ...
    while not done:
        ...
        state = next_state
        total_reward += reward
        #window.after(SPEED, perform_action, state, action)
    '''
    # הפחתת epsilon
...
def run_simulator():
    ...
    def run_episodes(i=0):
        loadQtable()  # Load the Q-table from a file
        ...
        if i < episodes:
            ...
            window.after(50000, run_episodes, i + 1)  # Schedule the next episode
        else:
            saveQtable()  # Save the Q-table after all episodes are complete
